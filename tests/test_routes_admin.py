...
class TestRoutesAdmin:
    ...
    def test_ajouter_capteur_champs_manquants(self, mock_service):
        ...
        # Arrange
        payloads_invalides = [
            {'nom': 'TEST'},  # Manque type_capteur et id_salle
            {'type_capteur': 'temperature'},  # Manque nom et id_salle
            {'id_salle': 1},  # Manque nom et type_capteur
            {'nom': 'TEST', 'type_capteur': 'temperature'},  # Manque id_salle
        ]
    ...
    def test_parametres_url_valides(self, mock_service):
        ...
        for capteur_id in [1, 10, 999]:
            ...
            # Le service sera appelé avec le bon ID même si il lève une exception
    ...
    def test_parametres_url_invalides(self):
        ...
        assert response.status_code == 404  # Route non trouvée
        
        # Test avec ID négatif (Flask l'accepte mais le service peut le rejeter)
        response = self.client.put('/api/admin/capteurs/-1/desactiver')
        assert response.status_code == 404  # Route non trouvée car <int:> rejette les négatifs
